...
static inline void put_pixel(uint32_t pixel_grb) {
    pio_sm_put(pio0, 0, pixel_grb << 8u);
}
...
void push_random_byte(){
    uint8_t rand_val = rand() & 0xFF;
    pio_sm_put(pio0, 0, rand_val);
}

void push_byte(uint8_t val){
    pio_sm_put(pio0, 0, (uint32_t)val);
}
...
int main() {
    ...
    adb_write_program_init(pio, sm, offset, ADB_PIN, 800000, IS_RGBW);
